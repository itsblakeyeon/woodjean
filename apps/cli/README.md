...
1. **CLI 실행** — 터미널에서 `npx woodjean@latest order` 한 줄. Node.js만 있으면 install 불필요.
...
향후 7일 슬롯이 모두 찼거나 매장이 일시 중지 상태일 수 있어요. 급하면 매장(010-8484-2120)으로 문의해 주세요.

먼저 `npx woodjean@latest --version`이 최신 npm 버전과 같은지 확인해 주세요. 예전에 `npm install -g woodjean`으로 설치했다면 `npx woodjean`이 전역 구버전을 실행할 수 있어요. 이때는 `npm uninstall -g woodjean` 후 다시 실행하거나, `npm exec --package woodjean@latest -- woodjean order`로 최신 패키지를 강제로 실행해 주세요.
